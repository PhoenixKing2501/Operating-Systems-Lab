...
int get_parent(pid_t pid)
{
    ...
    if (detectbug) // flag to check if user entered --suggest flag
    {
        ...
        int sus_flag = 0, non_sleep_time, total_time;
        ...
        if (fp)
        {
            ...
            if (fscanf(fp, "%*d %*s %*c %*d %*d %*d %*d %*d %*u %*u %*u %*u %*u %d %d %d %d", &utime, &stime, &cutime, &cstime) == 4)
            {
                total_time = utime + stime + cutime + cstime;
                non_sleep_time = utime + stime;
                // printf("Non sleep time of process %d is %d\n", pid, non_sleep_time);
                // printf("Total time of process %d is %d\n", pid, total_time);
            }
            fclose(fp);
        }
