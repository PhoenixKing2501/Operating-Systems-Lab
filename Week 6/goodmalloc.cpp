...
void fn_beg()
{
	std::fprintf(stdout, "fn_beg() called\n");
...
{
	std::fprintf(stdout, "createMem(%d) called\n", size);
	// Allocate memory
...
{
	std::fprintf(stdout, "createList(%s, %d) called\n", name, num_elements);
...
{
	std::fprintf(stdout, "assignVal(%s, %d, %d) called\n", list_name, idx, val);
	// Find the table row
...
{
	std::fprintf(stdout, "getVal(%s, %d) called\n", list_name, idx);
	// Find the table row
	int32_t row = T.find(list_name);
	if (row == -1)
	{
...
{
	std::fprintf(stdout, "freeElem(%s) called\n", list_name);
	// Find the table row
	int32_t row = T.find(list_name);
	if (row == -1)
		return 0;
...
int32_t freeElem()
{
	std::fprintf(stdout, "freeElem() called\n");
...
{
	std::fprintf(stdout, "printList(%s) called\n", list_name);
	// Find the table row
	int32_t row = T.find(list_name);
	if (row == -1)
		return;
...
void deleteMem()
{
	std::fprintf(stdout, "deleteMem() called\n");
	if (mem)
		delete[] mem;
}
...
{
	std::fprintf(stdout, "listBegin(%s) called", list_name);
	// Find the table row
	int32_t row = T.find(list_name);
	if (row == -1)
	{
		std::fprintf(stderr, "listBegin: %s not found\n", list_name);
		exit(EXIT_FAILURE);
	}
...
{
	std::fprintf(stdout, "listPtr(%s, %d) called\n", list_name, idx);
	// Find the table row
	int32_t row = T.find(list_name);
	if (row == -1)
	{
		fprintf(stderr, "listPtr: %s not found\n", list_name);
		exit(EXIT_FAILURE);
	}
...
{
	std::fprintf(stdout, "listEnd(%s) called\n", list_name);
	// Find the table row
	int32_t row = T.find(list_name);
	if (row == -1)
	{
		std::fprintf(stderr, "listEnd: %s not found\n", list_name);
		exit(EXIT_FAILURE);
	}
